(function($) {
    ...
    $.widget('R.carousel3d', {
         ...
         * @property {Number} options.shift Perspective shift 
         */
        options: {
            perspective: 500,
            duration: 1000,
        ...
        _layout: function() {
            ...
            $wrapper.css({
                transform: 'translateZ('+ -1 * (translateZ + z) +'px) translateY(-50%)'
            });

            if (this._listCnt == 3) {
                $wrapper.css({
                    transform: 'translateZ(' + (translateZ + z) +'px) translateY(-50%)'
                });
            }

            $list.each(function(i) {
                $(this).css({
                    transform: 'rotateY('+ (deg * (i + 0.5 + shift % 0.5)) +'deg) translateZ('+ translateZ +'px)'
                });
            });
            if ($list.length === 1) {
                $wrapper.css({
